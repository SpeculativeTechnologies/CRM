...
const ENGAGEMENT_FIELDS = [
  { universalIdentifier: MESSAGE.fields.openedAt.universalIdentifier, name: 'openedAt' },
  { universalIdentifier: MESSAGE.fields.openCount.universalIdentifier, name: 'openCount' },
  { universalIdentifier: MESSAGE.fields.clickedAt.universalIdentifier, name: 'clickedAt' },
  { universalIdentifier: MESSAGE.fields.clickCount.universalIdentifier, name: 'clickCount' },
  { universalIdentifier: MESSAGE_CAMPAIGN.fields.openedCount.universalIdentifier, name: 'openedCount' },
  { universalIdentifier: MESSAGE_CAMPAIGN.fields.clickedCount.universalIdentifier, name: 'clickedCount' },
];
...
describe('AddMessageEngagementTrackingFieldsCommand', () => {
  let command: AddMessageEngagementTrackingFieldsCommand;
  ...
  beforeEach(() => {
    ...
    command = new AddMessageEngagementTrackingFieldsCommand(
  ...
  it('creates every engagement field on a workspace missing them all', async () => {
    ...
    expect(validateBuildAndRunWorkspaceMigrationMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        allFlatEntityOperationByMetadataName: {
          fieldMetadata: {
            flatEntityToCreate: ENGAGEMENT_FIELDS.map((field) =>
  ...
  it('creates only the missing fields on a partially upgraded workspace', async () => {
    ...
    expect(validateBuildAndRunWorkspaceMigrationMock).toHaveBeenCalledWith(
      expect.objectContaining({
        allFlatEntityOperationByMetadataName: {
          fieldMetadata: {
            flatEntityToCreate: ENGAGEMENT_FIELDS.slice(2).map((field) =>
              expect.objectContaining(field),
            ),
            flatEntityToDelete: [],
            flatEntityToUpdate: [],
  ...
  it('skips an already-upgraded workspace without building a migration', async () => {
    getOrRecomputeMock.mockResolvedValue({
      flatFieldMetadataMaps: buildByUniversalIdentifierMap(
        ENGAGEMENT_FIELDS.map(({ universalIdentifier }) => universalIdentifier),
      ),
      flatObjectMetadataMaps: buildByUniversalIdentifierMap(
        ALL_OBJECT_UNIVERSAL_IDENTIFIERS,
      ),
