...
import { type CampaignEngagementCounts } from 'src/engine/core-modules/emailing-domain/types/campaign-engagement-counts.type';
...
export class MessageCampaignStatisticsService {
  ...
  async persistCampaignCounts({
    ...
    await this.workspaceOrmManager.executeInWorkspaceContext(async () => {
      ...
      const engagementCounts = await this.countEngagement(campaignId);
  ...
  // Engagement is counted per recipient, not per hit, so a recipient who opens
  // six times still moves the campaign open rate by one. It lives on the
  // message rows rather than the delivery rows because the mass-compose path
  // tracks through the mailbox message it persisted.
  private async countEngagement(
    campaignId: string,
  ): Promise<CampaignEngagementCounts> {
    ...
    return { openedCount, clickedCount, repliedCount };
  }
}
