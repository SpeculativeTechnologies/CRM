...
type LogContactProps = { presentation?: 'panel' | 'tab' };

export const LogContact = ({ presentation = 'panel' }: LogContactProps) => {
  ...
  const handleSave = async () => {
    ...
    // Panel saves stay disabled if closing fails, so retrying cannot duplicate them.
    if (presentation === 'tab') {
      resetForm();
      saveInProgress.current = false;
      setSaving(false);
